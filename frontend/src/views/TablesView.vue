<script setup lang="ts">
  ...
  const tableMap = computed(() => {
    ...
    for (const num of TABLES) {
      const tableOrders = orders.value
        .filter((o) => o.orderType === "dinein" && o.table === num)
        .sort((a, b) => a.id - b.id);
      map[num] =
        tableOrders.length > 0
          ? { num, occupied: true, orders: tableOrders }
          : { num, occupied: false };
    }
    return map;
  });
  ...
  <main class="mx-auto max-w-[1100px] px-5 pt-[26px] pb-[60px]">
    ...
    <div class="flex flex-col gap-8 lg:flex-row lg:items-start">
      <section class="flex-1">
        <div class="mb-3 text-xs font-extrabold text-[rgba(26,26,29,.5)]">
          前面
        </div>
